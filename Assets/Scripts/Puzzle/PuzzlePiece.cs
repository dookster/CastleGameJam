...
public class PuzzlePiece : MonoBehaviour {
    ...
    void OnMouseOver()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            {
                AudioPlayer.Instance.Play2DAudio(ownerPuzzle.exitPuzzle ? settings.exitSwitchAudio1 : settings.RandomSwitch);
        ...
        if (Input.GetMouseButtonUp(1))
        {
            if (iTween.Count(gameObject) == 0)
            {
                AudioPlayer.Instance.Play2DAudio(ownerPuzzle.exitPuzzle ? settings.exitSwitchAudio2 : settings.RandomSwitch);
                iTween.MoveBy(gameObject, iTween.Hash("y", moveLength, "time", moveTime));
                iTween.RotateBy(gameObject, iTween.Hash("y", - (1f / 4f), "time", rotateTime));
